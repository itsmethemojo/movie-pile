      ...
      yaml '''
        ...
        spec:
          ...
          - name: javascript
            ...
            tty: true
          - name: kaniko
            image: ghcr.io/osscontainertools/kaniko:v1.28.4-slim
            command:
            - cat
            tty: true
          nodeSelector:
    ...
    stage('build') {
      steps {
        container('kaniko') {
          ...
          sh '/kaniko/executor --dockerfile Dockerfile --context . --destination $CONTAINER_IMAGE_REGISTRY/$CONTAINER_IMAGE_PATH:amd64-$SHORT_COMMIT $(cat ~/.docker/push | xargs)'
        }
      }
    }
  }
